...
alter table Employee add foreign key(DepNo) references Department;
	
create table Department(
	Dname varchar(50),
	DepNo int,MgrSNN varchar(50),
	MgrSDate varchar(50),
	primary key (DepNo),
	foreign key(MgrSNN) references Employee(SSN));
...
create table Works_on(
	...
	foreign key (PNumber) references Project);
...
select DepNo,min(salary) from Employee group by DepNo;

select avg(salary) from Employee where salary>40000;

select count(DepNo) from Employee where DepNo = 6;

select DepNo, count(FName) from Employee where FName like 'J%' group by DepNo;

select DepNo, count(PNumber) from Project group by DePno;